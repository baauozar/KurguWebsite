...
{
    public class CaseStudyUniquenessChecker : ICaseStudyUniquenessChecker
    {
        private readonly KurguWebsiteDbContext _db;
        public CaseStudyUniquenessChecker(KurguWebsiteDbContext db) => _db = db;

        public Task<bool> SlugExistsAsync(string slug, Guid? excludeId = null, CancellationToken ct = default)
          => _db.CaseStudies.AsNoTracking()
